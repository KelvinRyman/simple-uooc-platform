...
# 数据库配置
db_password = '114514'
app.config['SQLALCHEMY_DATABASE_URI'] = r'mysql+mysqlconnector://root:[db_password]@localhost/web'
...
def register_user():
    ...
    # 添加到數據庫
    new_user = Users(id=str(uuid.uuid4()), username=username, password=password, role=role)
...
def register_course():
    ...
    # 防止重複註冊
    existing_enrollment = CourseEnrollments.query.filter_by(student_id=student_id, course_id=course_id).first()
...
def get_registered_courses():
    ...
    courses = db.session.query(Courses.title, Courses.id).join(CourseEnrollments, Courses.id == CourseEnrollments.course_id).filter(CourseEnrollments.student_id == student_id).all()
    return jsonify({'registered_courses': [{'id': c.id, 'title': c.title} for c in courses]})
...
def get_pending_assignments():
    student_id = request.args.get('student_id')
    assignments = db.session.query(Assignments.title, Assignments.id).join(Courses).join(CourseEnrollments).filter(CourseEnrollments.student_id == student_id).all()
    return jsonify({'pending_assignments': [{'id': a.id, 'title': a.title} for a in assignments]})
...
def get_submitted_assignments():
    ...
    return jsonify({
        'submitted_assignments': [
            {'assignment_id': s.assignment_id, 'content': s.content, 'file_url': s.file_url} for s in submissions
        ]
    })
